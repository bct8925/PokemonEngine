...
public class Pokeman {
  ...
  private void movement() {
    ...
    if (!player.isStopMoving() && !upKey.keyDown() && !downKey.keyDown() && !leftKey.keyDown()
        && !rightKey.keyDown()) {
  ...
  public void doorZone(Tile tile) {
    ...
    zoneChosen = false;
    gamePanel.fOut();
    AudioManager.playMP3("/audio/door.mp3", false);
    ...
    if (!found) {
      curZone = new Zone(this, tile.getDoorZone(),
          getClass().getResourceAsStream("/zones/" + tile.getDoorZone()));
      curZone.drawZone();
    }
  ...
  private void init() {
    try {
      player = new Player(this, new Sprite(ImageIO.read(getClass().getResourceAsStream(
          "/sprites/player.png")), 24, 16));
    } catch (IOException e) {
      e.printStackTrace();
    }
    player.centerBox(windowWidth / 2, windowHeight / 2);    // Set player position

    curZone = new Zone(this, "littleroot.zone",
        getClass().getResourceAsStream("/zones/littleroot.zone"));
    prevZone = curZone;
    Thread loadThread = new Thread(() -> {
      zonesLoaded = false;
      linkedZones = curZone.getLinkedZones();
    ...
    AudioManager.playMP3("/audio/music.mp3", true);
  }
