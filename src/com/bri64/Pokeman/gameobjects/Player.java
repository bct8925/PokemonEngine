...
public class Player extends Box {
    ...
    public void move(int dir, boolean sprint) {     // Player movement
        if (!moving && direction != dir) {  // If not moving and current direction is not the new direction, turn
            ...
            animating = true;
            ani.run(() -> aniTurn(dir, ani.getFrame(), main.getCurZone().getTileZoom() / 2), main.getCurZone().getTileZoom() / 2);  // Run turning animation
            direction = dir;    // Set new direction
        }
        else {  // Otherwise, walk
            direction = dir;    // Set new direction
            if (dir == 0 && !main.getCurZone().getTile(location[0], location[1] - 1).isSolid()) {         // If moving up and new tile is not solid
                ...
                if (!sprint) {  // If not sprinting
                    ani.run(() -> aniMove(dir, ani.getFrame(), main.getCurZone().getTileZoom()), main.getCurZone().getTileZoom());          // Start walking animation
                } else {        // If sprinting
                    ani.run(() -> aniRun(dir, ani.getFrame(), main.getCurZone().getTileZoom() / 2), main.getCurZone().getTileZoom() / 2);   // Start running animation
                }
            }
            else if (dir == 1 && !main.getCurZone().getTile(location[0], location[1] + 1).isSolid()) {   // If moving down and new tile is not solid
                moving = true;
                if (animatingWall) {
                    ani.stopAnimation();
                    animatingWall = false;
                ...
                if (!sprint) {
                    ani.run(() -> aniMove(dir, ani.getFrame(), main.getCurZone().getTileZoom()), main.getCurZone().getTileZoom());
                } else {
                    ani.run(() -> aniRun(dir, ani.getFrame(), main.getCurZone().getTileZoom() / 2), main.getCurZone().getTileZoom() / 2);
                }
            }
            else if (dir == 2 && !main.getCurZone().getTile(location[0] - 1, location[1]).isSolid()) {    // If moving left and new tile is not solid
                moving = true;
                if (animatingWall) {
                    ani.stopAnimation();
                    animatingWall = false;
            ...
            else if (dir == 3 && !main.getCurZone().getTile(location[0] + 1, location[1]).isSolid()) {    // If moving right and new tile is not solid
                moving = true;
                if (animatingWall) {
                    ani.stopAnimation();
                    animatingWall = false;
                ...
                if (!animatingWall) {   // If not already walking into a wall, do it
                    ani = new Animation();
                    animatingWall = true;
                    ani.run(() -> aniWall(dir, ani.getFrame(), main.getCurZone().getTileZoom() * 2), main.getCurZone().getTileZoom() * 2);  // Start wall animation
                }
            }
        }
    }
