...
public class Zone {
  ...
  public Zone(Pokeman main, String name, InputStream in) {
    ...
    try {
      this.tileset = ImageIO.read(getClass().getResourceAsStream("/sprites/" + tileMap));
    } catch (IOException e) {
      System.exit(1);
    }
    ...
    for (String cell : zoneCells.split(";")) {
      ...
      tile[x / tileSize][y / tileSize] = new Tile(x / tileSize, y / tileSize, solid, layer,
          property, tileset.getSubimage(imgX, imgY, tileSize, tileSize));
  ...
  public void drawZone() {
    image = new BufferedImage(row * getTileZoom(), col * getTileZoom(),
        BufferedImage.TYPE_INT_ARGB);
    Graphics2D bg = (Graphics2D) image.getGraphics();
    //bg.fillRect(0, 0, image.getWidth(), image.getHeight());   // Fill image
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < col; j++) {
        bg.drawImage(tile[i][j].getImage()
                .getScaledInstance(getTileZoom(), getTileZoom(), Image.SCALE_SMOOTH), i * getTileZoom(),
            j * getTileZoom(), main.getGamePanel());
      }
    }
  }
    ...
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < col; j++) {
        if (tile[i][j].isDoor() && !tile[i][j].getDoorZone().equals(main.getPrevZone().getName())) {
          linkedZones.add(new Zone(main, tile[i][j].getDoorZone(),
              getClass().getResourceAsStream("/zones/" + tile[i][j].getDoorZone())));
        }
      }
    }
